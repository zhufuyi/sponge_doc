...
`⓷基于protobuf创建的web服务`是用于通用web服务开发，支持自选数据库类型作为数据存储。而`⓵基于sql创建的web服务`，也就是[web开发(mysql)](https://go-sponge.com/zh-cn/web-development-mysql)已经选定mysql数据库类型，这是两种方式创建web服务的主要区别之一，可以把`⓵基于sql创建的web服务`看作是`⓷基于protobuf创建的web服务`一个特殊子集。
