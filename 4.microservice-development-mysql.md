...
如果没有`goland` IDE，也可以通过命令来测试，切换到`internal/service`目录，打开后缀为`_client_test.go`的测试文件，修改rpc方法的请求参数，执行测试命令`go test -run 测试函数名/rpc方法名`，示例： `go test -run Test_service_teacher_methods/GetByID`。
...
如果在本服务需要调用其他微服务的api接口，请按照下面步骤进行操作。

**(1) 添加连接目标微服务代码**

想要在本服务内调用目标微服务api接口，首先要能够连接上目标微服务，下面自动生成rpc连接代码。

进入sponge的UI界面，点击左边菜单栏【Public】-->【生成rpc服务连接代码】，填写module名称，填写rpc服务名称(支持多个rpc服务名称，用逗号分隔)，填写完参数后，点击按钮`下载代码`生成rpc服务连接代码，如下图所示：
...
> [!tip] rpc连接代码其实是grpc客户端连接代码，包括了服务发现、负载均衡、安全连接、链路跟踪、指标采集等设置，也可以添加自己定义的连接设置。

解压代码，把目录`internal`移动到本服务代码目录下。

> [!note] 移动目录`internal`到本服务目录下正常情况下不会有冲突文件，如果有冲突文件，说明之前已经指定相同的微服务名称来生成rpc服务连接代码了，此时忽略覆盖文件。
...
把目标微服务代码目录的`api/目标微服务名称/v1/xxx.proto`文件复制出来，并移动到本服务的`api`目录。有了目标微服务proto文件，在本服务就可以知道有哪些api接口可以调用了。
