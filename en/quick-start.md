...
> [!note] Go version 1.16 or higher is required. Add the directory where the binary files generated by `go install` are located (usually $GOPATH/bin) to the system's PATH environment variable.
...
> [!note] Version 3.20 or higher is required. Add the directory containing the `protoc` binary file (recommended to be $GOPATH/bin) to the system's PATH environment variable.
...
Install sponge in a macOS environment with the following commands:
...
Install sponge in a Linux environment with the following commands:
...
> [!note] sponge's UI interface only supports code generation on the local machine and does not support cross-IP access. This limitation arises from the fact that sponge compiles both frontend files (HTML, JS, CSS) and backend services into the sponge executable. The frontend JS code uses the default IP address `127.0.0.1`, which cannot be changed. If you need to deploy the sponge UI service remotely, you can download the sponge source code, search for the `spongeServiceAddr` field, and configure it with your IP address or domain name. Finally, compile a new sponge executable and deploy it on the target server.
