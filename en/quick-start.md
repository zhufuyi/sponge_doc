...
Add `GOPATH/bin` to the system path, skip this step if already set.

```bash
# Check if GOPATH is set.
go env GOPATH

# If empty, set the GOPATH directory, it may be necessary to execute this command with administrator privileges.
go env -w GOPATH=your directory (example D:\golang\package)

# Add the bin directory under GOPATH (example D:\golang\package\bin) to the system path.
```

<br>

Execute the command to install sponge, sponge and dependent plugins will be installed to the `GOPATH/bin` directory.
...
# Check if all plugins have been successfully installed. If you find that some plugins haven't been installed successfully, execute the following command to retry: sponge tools --install
sponge tools
...
# If empty, set the GOPATH directory.
go env -w GOPATH=your-directory(example ~/golang/package)

# Add the bin directory under GOPATH (example ~/golang/package/bin) to the system path.
echo 'export PATH=$PATH:your-directory' >> ~/.bashrc
source ~/.bashrc
```
