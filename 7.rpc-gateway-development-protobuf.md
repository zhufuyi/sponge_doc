...
> [!tip] 解压的rpc网关服务代码目录名称的格式是`服务名称-类型-时间`，可以修改目录名称(例如把名称中的类型和时间去掉)。
...
想要在rpc网关服务里调用微服务api接口，首先要能够连接上微服务，下面自动生成rpc连接代码。
...
> [!note] 移动目录`internal`到rpc服务目录下正常情况下不会有冲突文件，如果有冲突文件，说明之前已经指定相同的微服务名称来生成rpc服务连接代码了，此时忽略覆盖文件。
...
虽然在rpc网关服务可以连接到微服务，但是不知道微服务哪些api接口可以调用，通过proto文件可以告诉rpc网关服务可以调用的api接口。

把微服务代码目录的`api/微服务名称/v1/xxx.proto`文件复制出来，并移动到rpc网关服务代码的`api`目录。有了微服务proto文件，rpc网关服务就可以知道有哪些api接口可以调用了。

切换到rpc网关服务目录，打开终端，执行命令：
