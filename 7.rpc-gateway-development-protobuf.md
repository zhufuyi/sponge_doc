...
虽然在rpc网关服务可以连接到微服务，但是不知道微服务哪些api接口可以调用，通过proto文件可以告诉grpc网关服务可以调用的api接口。
...
> [!note] `在proto文件定义api接口描述信息`是在属于grpc网关服务自己的目录上添加，例如这里目录是`api/edusys/v1`，在该目录下的proto文件添加api接口描述信息，也可以创建新的proto文件，然后添加api接口描述信息。
