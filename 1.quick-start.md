...
进入sponge的UI界面，点击左边菜单栏【SQL】--> 【创建web项目】，填写`mysql dsn地址`，然后点击`获取表名`，接着填写其他参数，鼠标放在问号`?`位置查看参数说明，填写完参数后，点击按钮`下载代码`生成web服务完整项目代码，如下图所示：
...
`如果有新的mysql表，如何批量生成标准化的CRUD api接口代码无缝嵌入到项目代码呢？如果想新增的自定义api接口，又如何操作呢？在[web开发(mysql)](https://go-sponge.com/zh-cn/web-development-mysql)章节中详细介绍。
