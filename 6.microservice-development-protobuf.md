...
如果没有`goland` IDE，也可以通过命令来测试，切换到`internal/service`，打开后缀为`_client_test.go`的测试文件，修改rpc方法的请求参数，执行测试命令`go test -run 测试函数名/rpc方法名`，示例： `go test -run Test_service_teacher_methods/GetByID`。
